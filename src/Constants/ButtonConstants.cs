namespace form_builder.Constants
{
    public class ButtonConstants
    {
        public static readonly string NEXTSTEP_TEXT = "Continue";
        public static readonly string SUBMIT = "Submit";
        public static readonly string SUBMIT_TEXT = "Submit";
        public static readonly string ADDRESS_SEARCH_TEXT = "Find address";
        public static readonly string STREET_SEARCH_TEXT = "Find street";
        public static readonly string ORG_SEARCH_TEXT = "Find organisation";
        public static readonly string UPLOAD_FILES_TEXT = "Upload file(s)";
    }
}
