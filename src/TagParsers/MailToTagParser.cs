...
public class MailToTagParser(IEnumerable<IFormatter> formatters) : TagParser(formatters), ITagParser, ISimpleTagParser
{
    public Regex Regex => new("(?<={{)MAILTO:.+?[@].+?(?=}})", RegexOptions.Compiled);
