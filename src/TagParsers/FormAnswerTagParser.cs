...
public class FormAnswerTagParser(IEnumerable<IFormatter> formatters) : TagParser(formatters), ITagParser
{
    public Regex Regex => new("(?<={{)QUESTION:.*?(?=}})", RegexOptions.Compiled);
