...
public class EmphasisTagParser : TagParser, ITagParser
{
    public EmphasisTagParser(IEnumerable<IFormatter> formatters) : base(formatters) { }

    public Regex Regex => new Regex("(?<={{)EMPHASIS::.*?(?=}})", RegexOptions.Compiled);
