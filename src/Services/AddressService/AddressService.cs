    ...
    public class AddressService : IAddressService
    {
            ...
            string path)
        {
            ...
            else
            {
                try
                {
                    addressResults = (await _addressProviders.Get(addressElement.Properties.Provider).SearchAsync(postcode)).ToList<object>();
                }
                catch (Exception e)
                {
                    throw new ApplicationException($"AddressService::ProcessSearchAddress, An exception has occurred while attempting to perform postcode lookup on Provider '{addressElement.Properties.Provider}' with searchterm '{postcode}' Exception: {e.Message}", e);
                }
