    ...
    public class BookingElementCheck : IElementSchemaIntegrityCheck
    {
        ...
        public IntegrityCheckResult Validate(IElement element)
        {
            ...
            if (string.IsNullOrEmpty(element.Properties.BookingProvider))
                integrityCheckResult.AddFailureMessage($"Booking Element Check, Booking element '{element.Properties.QuestionId}' requires a valid booking provider property on form {schema.FormName}.");

            var appointmentTypeForEnv = element.Properties.AppointmentTypes.FirstOrDefault(_ => _.Environment.ToLower().Equals(_environment.EnvironmentName.ToLower()));
