            ...
            foreach (var behaviour in behaviours.Where(behaviour => behaviour.SubmitSlugs.Any()))
            {
                foreach (var submitSlug in behaviour.SubmitSlugs)
                {
                    if (string.IsNullOrEmpty(submitSlug.URL))
                        integrityCheckResult.AddFailureMessage($"No URL found for SubmitSlug in environmment '{submitSlug.Environment}'");

                    if (string.IsNullOrEmpty(submitSlug.AuthToken))
                        integrityCheckResult.AddFailureMessage($"No auth token found for SubmitSlug in environmment '{submitSlug.Environment}'");
