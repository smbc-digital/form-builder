    ...
    public class BookingElementCheck : IElementSchemaIntegrityCheck
    {
        ...
        public IntegrityCheckResult Validate(IElement element)
        {
            ...
            if (appointmentTypeForEnv is null)
            {
                result.AddFailureMessage(
                    "Booking Element Check, " +
                    "No AppointmentType found for current environment.");
                ...
                string.IsNullOrEmpty(appointmentTypeForEnv.AppointmentIdKey))
            {
                result.AddFailureMessage(
                    "Booking Element Check, " +
                    "You must supply either an AppointmentId or an AppointmentIdKey in the AppointmentType.");
            }
                ...
                !string.IsNullOrEmpty(appointmentTypeForEnv.AppointmentIdKey))
            {
                result.AddFailureMessage(
                    "Booking Element Check, " +
                    "You cannot use both AppointmentId and AppointmentIdKey in the AppointmentType.");
            }
            ...
            foreach (var resource in appointmentTypeForEnv.OptionalResources)
            {
                if (resource.Quantity <= 0)
                {
                    result.AddFailureMessage(
                        "Booking Element Check, " +
                        $"Booking element '{element.Properties.QuestionId}', " +
                        "optional resources are invalid, cannot have a quantity of 0 or less.");
                }
                ...
                if (resource.ResourceId.Equals(Guid.Empty))
                {
                    result.AddFailureMessage(
                        "Booking Element Check, " +
                        $"Booking element '{element.Properties.QuestionId}', " +
                        "optional resources are invalid, ResourceId cannot be an empty Guid.");
                }
            }
