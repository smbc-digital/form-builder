...
namespace form_builder.Validators.IntegrityChecks.Form
{
    public class SummaryElementFormCheck : IFormSchemaIntegrityCheck
    {
        public IntegrityCheckResult Validate(FormSchema schema)
        {
            ...
            var summaryElements = schema.Pages
                .SelectMany(page => page.Elements
                    .Where(element => element.Type
                        .Equals(EElementType.Summary)))
                .ToList();
                    ...
                    properties.Sections.ForEach(_ =>
                    {
                        if (string.IsNullOrEmpty(_.Title))
                        {
                            result.AddFailureMessage(
                                "FormSchemaIntegrityCheck::" +
                                "SummaryFormCheck, Summary section is defined but section title is empty. " +
                                "Please add a title for the section.");
                        }
                        ...
                        if (_.Pages is null || _.Pages.Count.Equals(0))
                        {
                            result.AddFailureMessage(
                                "FormSchemaIntegrityCheck::" +
                                "SummaryFormCheck, " +
                                "Summary section is defined but no pages have been specified to appear in the section.");
                        }
                            ...
                            _.Pages.ForEach((x) =>
                            {
                                if (!schema.Pages.Any(_ => _.PageSlug.Equals(x)))
                                {
                                    result.AddFailureMessage(
                                        "FormSchemaIntegrityCheck::" +
                                        "SummaryFormCheck, " +
                                        "Summary section has a page slug defined which cannot be found. " +
                                        $"Verify '{x}' is a valid page slug within the schema.");
                                }
                            });
                        }
                    });
